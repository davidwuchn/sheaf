...
then grows a new layer at runtime when it detects a loss plateau, resuming training without restarting.

In JAX or PyTorch, this requires retracing the computation graph and reinitializing the optimizer. Because Sheaf is a homoiconic language, the model is data itself: changing its shape mid-loop is no different from updating a dictionary.
