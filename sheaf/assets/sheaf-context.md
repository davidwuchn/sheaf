...
Pure functions are compiled automatically on first call. No explicit build step is needed.

Compiled artifacts (VMFB + `manifest.json`) are cached in the `__sheaf__/` directory beside the source file. The manifest records function signatures and content hashes; stale entries are recompiled automatically.

The compiler toolchain (`iree-compile`) is downloaded to `~/.sheaf/toolchain/` on first use.
...
├── src/
│   ├── bin/sheaf.rs       # CLI: repl, run, init-ai
