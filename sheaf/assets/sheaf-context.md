---
title: Sheaf Language Reference
version: 2.0
target: AI Assistants & LLM Agents
last_updated: 2026-03-21
keywords:
  [sheaf, stablehlo, iree, lisp, neural-networks, ml, dsl, differentiable]
note: "This file is auto-generated by 'sheaf init-ai'"
---

# Sheaf Language Reference (AI Assistant Context)

> **Quick reference for AI assistants helping with Sheaf development.**
>
> Sheaf is a differentiable Lisp dialect for machine learning. Philosophy:
> "Clojure for Tensors". It compiles pure S-expressions into StableHLO and
> executes them via the IREE runtime (statically linked in the sheaf binary).

---

## How This Document Works

**This document (when generated by `sheaf init-ai`) is a language guide.**

It covers concepts, patterns, training strategies, and debugging.

**For function signatures and source**, use the built-in REPL commands:

```bash
echo ':list' | sheaf 2>/dev/null         # All functions by category
echo ':h <name>' | sheaf 2>/dev/null     # Signature + examples for one function
echo ':show <name>' | sheaf 2>/dev/null  # Source of any defn - inspect stdlib impls (e.g. :show silu)
echo ':env' | sheaf 2>/dev/null          # All defined functions and variables
```

`:show` is the right way to inspect a stdlib function's implementation - unlike
`:h` (signature + prose), it returns the actual source.

---

## Quick Start Example

```sheaf
;; Simple MLP forward pass with parameter destructuring
(defn forward [x p]
  (as-> x h
    (with-params [p :l1]
      (relu (+ (@ h W) b)))
    (with-params [p :l2]
      (sigmoid (+ (@ h W) b)))))

;; Training step with Adam optimizer
(defn train-step [p m v t x y lr]
  (let [loss-fn (fn [params] (mse-loss (forward x params) y))
        [loss grads] ((value-and-grad loss-fn) p)
        [new-p new-m new-v new-t] (adam-step p grads m v t lr 0.9 0.999 1e-8)]
    {:p new-p :m new-m :v new-v :t new-t :loss loss}))
```

---

## Core Concepts

### 1. Everything is Differentiable

- All operations compile to StableHLO (MLIR-based tensor IR)
- Full gradient support via `(value-and-grad func)`
- Pure functions are AOT-compiled and dispatched via the IREE runtime

### 2. Dictionary-Based Parameters

Neural network parameters are stored in nested dictionaries:

```sheaf
{:l1 {:W weights :b biases}
 :l2 {:W weights :b biases}}
```

### 3. Compilation Pipeline

Sheaf programs follow a two-mode execution model:

- **Interpreter**: evaluates all Sheaf code (control flow, IO, side effects)
- **Compiler**: pure functions marked for compilation go through:
  `Sheaf → CompiledExpr → StableHLO (MLIR) → IREE VMFB → native execution`

The `(use module)` form automatically detects compiled artifacts (`.vmfb` +
`manifest.json`) and dispatches pure function calls through the IREE runtime
transparently.

### 4. Macro System

Sheaf supports compile-time macros with quasiquote syntax:

```sheaf
(defmacro when [cond body]
  `(if ~cond ~body nil))

;; Usage:
(when (> x 0)
  (print "positive"))

;; Expands to:
(if (> x 0)
  (print "positive")
  nil)
```

**Quote and quasiquote operators:**

- `'expr` or `(quote expr)` - quote: prevent evaluation, return data as-is
- `` `expr `` - quasiquote: create template with selective evaluation
- `~expr` - unquote: evaluate expression inside quasiquote
- `~@expr` - unquote-splicing: splice list into quasiquote

**Critical Distinction: Quote Before Square Brackets**

The quote prefix changes semantics fundamentally:

- `[1 2 3]` (unquoted) → Tensor (evaluated immediately to numeric array)
- `'[1 2 3]` (quoted) → Sheaf list (unevaluated data, treated as static
  structure)

Always quote shapes for functions that expect static data:

- Correct: `(ones '[10 20])`, `(reshape x '[2 -1])`
- Wrong: `(ones [10 20])`, `(reshape x [2 -1])` (these create tensors first!)

**Quote examples:**

```sheaf
'foo              ; => foo (symbol, not evaluated)
'(+ 1 2)          ; => (+ 1 2) (list, not evaluated)
`(+ 1 ~(* 2 3))   ; => (+ 1 6) (quasiquote with unquote)
```

**Standard macros** (from `lib/macros.shf`):

- `when` - conditional execution (returns nil if false)
- `unless` - inverse conditional
- `comment` - comment out code blocks
- `defmodel` - define a neural network as a sequence of named layers
- `defbatch` - define a function that auto-vectorizes over batch dimensions via
  `vmap`

---

## Syntax at a Glance

### Literals

```sheaf
[1 2 3]          ; Vector literal (tensor when numeric, tuple otherwise)
[D D]            ; Shape literal (evaluates to tuple when symbols present)
[[1 2] [3 4]]    ; Nested vectors (2D tensor)
{:a 1 :b 2}      ; Dictionary (like Clojure/Python)
{"key" value}    ; Dictionary with string keys
:keyword         ; Keyword (evaluates to string "keyword")
true / false     ; Booleans (lowercase)
nil              ; None
...              ; Ellipsis (for indexing/einsum)
```

**Bracket contexts:**

- `[]` in **binding context** (first arg to defn, let, fn): destructuring
  pattern
- `[]` in **expression context** (function arguments, return values): data
  literal

### Core Operators

**Function Definition**

```sheaf
(defn name [args] body ...)           ; Standard function (implicit do)
(fn [args] body ...)                  ; Anonymous function (implicit do)
```

**Implicit do:** `defn`, `fn`, and `let` accept multiple body expressions. All
are evaluated in order; the last value is returned. No explicit `(do ...)`
wrapper needed.

**Binding & Scope**

```sheaf
(def name value)                     ; Global immutable constant
(let [x val y val2] body ...)        ; Sequential local bindings (implicit do)
(with-params params body)            ; Auto-destructure dict (:W, :b, etc.)
```

**Control Flow**

```sheaf
(if cond then else)                  ; Branching
(case expr clause1 clause2 ...)      ; Pattern matching
(where cond true-val false-val)      ; Differentiable select (tensor-level)
(repeat [i n] [acc init] body)       ; Loop with accumulator
(while cond [acc init] body)         ; Conditional loop
(scan fn init xs)                    ; Fold with intermediate results
```

**Macros**

```sheaf
(defmacro name [params] template)    ; Define compile-time macro
'expr / (quote expr)                 ; Quote: prevent evaluation
`expr                                ; Quasiquote: create template
~expr                                ; Unquote: evaluate in template
~@expr                               ; Unquote-splicing: splice list in template
```

**Functional Operations**

```sheaf
(map func coll)                      ; Apply function to each element
(reduce func [acc] coll)             ; Reduce collection with optional accumulator
(apply func args-list)               ; Apply function to list of arguments
(vmap func)                          ; Vectorized map — auto-batch over axis 0
(vmap func axis)                     ; Vectorized map over specified axis
(vmap func [0 nil])                  ; vmap first arg on axis 0, keep second fixed
```

**Threading Macros**

```sheaf
(-> x (f1) (f2 a))                   ; Thread-first: (f2 (f1 x) a)
(as-> init name step1 step2)         ; Thread-as: bind value with custom name at each step
(->> x (map f) (filter pred))        ; Thread-last: (filter pred (map f x))
```

**Threading Macros in Detail:**

Threading macros help avoid deeply nested function calls. Choose based on your
data flow:

- **`->`** (thread-first): Inserts value as **first argument**

  ```sheaf
  (-> x (relu) (reshape '(10 -1)) (transpose '[1 0]))
  ; Equivalent to:
  (transpose (reshape (relu x) '(10 -1)) '[1 0])
  ```

- **`->>`** (thread-last): Inserts value as **last argument**

  ```sheaf
  (->> (range 10) (map (fn [x] (* x 2))) (reduce + 0))
  ; Equivalent to:
  (reduce + 0 (map (fn [x] (* x 2)) (range 10)))
  ```

- **`as->`** (thread-as): Binds value to **custom variable** at each step
  ```sheaf
  (as-> (forward x params) output
    (reshape output '[batch -1])
    (softmax output :axis -1)
    (mean output :axis 0))
  ```

---

## Common Patterns

### Pattern 1: Multi-Layer Perceptron

```sheaf
(defn forward [x p]
  (as-> x h
    (with-params [p :l1]
      (relu (+ (@ h W) b)))
    (with-params [p :l2]
      (sigmoid (+ (@ h W) b)))))
```

### Pattern 2: Token & Position Embeddings

```sheaf
;; Embedding = table lookup via (get table indices)
;; No nn.Embedding class needed: a tensor IS the embedding table.
;; No .long() needed: [0 1 2] works directly as indices.

(let [;; Params: token table [V, D], position table [B, D]
      tok-table (get params :token)   ;; [vocab_size, d_model]
      pos-table (get params :pos)     ;; [block_size, d_model]
      T (get (shape inputs) -1)

      ;; Lookup: (get table indices) gathers rows by index
      tok-emb (get tok-table inputs)                      ;; [Batch, T] -> [Batch, T, D]
      pos-emb (reshape (get pos-table (range T)) [1 T D]) ;; [T, D] -> [1, T, D]

      X (+ tok-emb pos-emb)]
  X)
```

### Pattern 3: Transformer Block

```sheaf
(defn transformer-block [x block-p config]
  (as-> x h
    (+ h (attention (layer-norm h (get block-p :ln1) -1)
                    (get block-p :attn) config))
    (+ h (mlp-block (layer-norm h (get block-p :ln2) -1)
                    (get block-p :mlp)))))
```

### Pattern 4: Training Loop with Adam

```sheaf
(defn train-step [params m v t inputs targets config]
  (let [lr (get config :lr)
        loss-fn (fn [p] (cross-entropy-loss (model inputs p config) targets))
        [loss grads] ((value-and-grad loss-fn) params)
        [new-params new-m new-v new-t] (adam-step params grads m v t lr 0.9 0.999 1e-8)]
    {:loss loss :params new-params :m new-m :v new-v :t new-t}))
```

### Pattern 5: Einsum for Multi-Head Attention

```sheaf
;; Q, K, V projections [Batch, Heads, Time, Head_dim]
(let [Qh (einsum "... t d, d h k -> ... h t k" X Wq_multi)
      Kh (einsum "... t d, d h k -> ... h t k" X Kh_multi)
      Vh (einsum "... t d, d h k -> ... h t k" X Vh_multi)]
  ...)
```

### Pattern 6: Batching with vmap

```sheaf
;; Define single-sample forward pass
(defn forward-single [x params]
  (with-params [params]
    (sigmoid (+ (@ x W) b))))

;; Batch it automatically with vmap
(let [batch-forward (vmap forward-single [0 nil])
      batch-results (batch-forward X params)]
  batch-results)
```

### Complete Working Examples

These are tested, runnable examples from the Sheaf repository:

- **MLP**:
  [model](https://raw.githubusercontent.com/sheaf-lang/sheaf/main/examples/mlp/mlp.shf)
  /
  [training](https://raw.githubusercontent.com/sheaf-lang/sheaf/main/examples/mlp/run.shf)
- **NanoGPT** (small Transformer):
  [model](https://raw.githubusercontent.com/sheaf-lang/sheaf/main/examples/nanoGPT/model.shf)
  /
  [inference](https://raw.githubusercontent.com/sheaf-lang/sheaf/main/examples/nanoGPT/sample.shf)
  /
  [training](https://raw.githubusercontent.com/sheaf-lang/sheaf/main/examples/nanoGPT/train.shf)

---

## Practical Training Guide

This section provides complete, production-ready patterns for training neural
networks in Sheaf.

### Training Checklist

The essential training pattern in Sheaf:

```sheaf
(defn train-step [params m v t x y lr]
  (let [loss-fn (fn [p] (mse-loss (forward x p) y))
        [loss grads] ((value-and-grad loss-fn) params)
        [new-p new-m new-v new-t] (adam-step params grads m v t lr 0.9 0.999 1e-8)]
    {:params new-p :m new-m :v new-v :t new-t :loss loss}))
```

**Hyperparameters:** Start with `lr=0.01`, Adam betas `0.9/0.999`. If loss is
NaN, reduce lr to `0.001` and check weight initialization.

**If training doesn't converge:**

- Loss is NaN → use proper initialization (`xavier-normal` for tanh/sigmoid,
  `kaiming-normal` for relu/gelu), reduce learning rate
- Loss is constant → run with `--guard` to catch NaN/Inf in function return
  values (no code changes):
  ```bash
  sheaf train.shf --guard no-nan              # check all functions
  sheaf train.shf --guard forward:no-nan      # check specific function
  sheaf train.shf --guard loss:range:0:20     # check value stays in range
  ```
- Use `--trace` to inspect shapes and values at each function call
- Last resort: `(clip-by-global-norm grads 1.0)` to cap gradient magnitude

See `:h adam-step`, `:h sgd-step`, `:h cross-entropy-loss`, `:h mse-loss`,
`:h xavier-normal`, `:h kaiming-normal`, etc. for detailed signatures.

---

## I/O: Loading, Saving, Reading

All file operations go through the single `(io verb ...)` form. There is no
`open`, no file handles — every call is a one-liner.

### Verb table

| Verb      | Signature                 | Returns         | Notes                                  |
| --------- | ------------------------- | --------------- | -------------------------------------- |
| `load`    | `(io "load" path)`        | pytree / handle | `.pkl`, `.safetensors`, `.npy`, `.bin` |
| `save`    | `(io "save" path params)` | nil             | Format inferred from extension         |
| `read`    | `(io "read" path)`        | string          | Whole file into memory                 |
| `lines`   | `(io "lines" path)`       | list of strings | One element per line                   |
| `exists`  | `(io "exists" path)`      | bool            | Check before load                      |
| `entropy` | `(io "entropy")`          | int             | 4 random bytes from OS. Use as seed    |

### Loading weights

```sheaf
(let [path "weights.pkl"
      params (if (io "exists" path)
               (io "load" path)
               (init-params key))]
  params)
```

### Saving after training

```sheaf
(io "save" "weights.pkl" params)
```

### Reading text data

```sheaf
(let [text (io "read" "data/shakespeare.txt")]
  text)

(let [lines (io "lines" "data/shakespeare.txt")]
  (len lines))
```

### Loading training data — NpyHandle & ShardedHandle

Large datasets should live in `.npy` or `.bin` files. `load` returns a lazy
handle backed by mmap — nothing is read until you slice.

```sheaf
;; Single .npy file → NpyHandle
(let [dataset (io "load" "data/train.npy")]
  (dataset 0)                                 ; first row
  (dataset 0:32))                             ; mini-batch

;; Many shards → ShardedHandle (virtual concat)
(let [tokens (io "load" "tokens/shard-*.bin" :i32)]
  (len tokens)                                ; total token count
  (tokens 0:4096))                            ; first 4096 tokens
```

Key rules:

- `.npy`: dtype from file header. No flag needed.
- `.bin`: dtype flag mandatory (`:i32`, `:f32`, `:f16`, `:bf16`, `:i16`, `:u32`,
  `:bool`).
- Glob patterns sorted lexicographically. Zero-pad shard names.

### Entropy for non-deterministic seeds

```sheaf
(let [key (random-key (io "entropy"))]
  (random-normal key '[10]))
```

### Full load-or-train pattern

```sheaf
(let [weights-path "weights.pkl"
      params (if (io "exists" weights-path)
               (do
                 (print "Loading weights...")
                 (io "load" weights-path))
               (do
                 (print "Training...")
                 (let [trained (train data config epochs)]
                   (io "save" weights-path trained)
                   trained)))]
  (generate params config))
```

---

## Gotchas & Common Mistakes

### Scalars vs Arrays

```sheaf
; Scalars are auto-converted
(+ 5 x)              ; 5 becomes a scalar array
(* 3.14 tensor)      ; 3.14 becomes a scalar array

; To extract a scalar from a tensor (e.g. for indexing):
(int (argmax [3 1 4]))       ; => 2
(float (sum [1 2 3]))        ; => 6.0
```

### Unary Negation

```sheaf
(- x)                   ; Negates x (unary minus)
(- a b)                 ; Subtracts b from a (binary minus)
```

### Equality Operators

```sheaf
(= a b)      ; Global equality → true/false (use in `if`)
(== a b)     ; Element-wise equality → tensor of bools (use for masks)
(!= a b)     ; Element-wise inequality
```

### Shape Inference with Static & Reshape

```sheaf
(reshape (arange 12) '[3 -1])     ; => shape [3, 4] - infers 4
(reshape (arange 12) '[2 -1 2])  ; => shape [2, 3, 2] - infers 3
```

### Dictionary Access

```sheaf
(get dict :key)                      ; Single key access
(get dict :missing 99)               ; With default value
(get-in dict [:path :to :key])       ; Nested access
(get-in dict [:path :missing] 99)    ; Nested with default
(assoc dict :k1 v1 :k2 v2)          ; Add/update keys (functional)
(dissoc dict [:k1 :k2])             ; Remove keys (functional)
(merge dict1 dict2)                  ; Merge dicts (later overrides)
(keys dict)                          ; Get all keys as list
(vals dict)                          ; Get all values as list
(with-params [dict] body)            ; Auto-bind :W, :b, etc. as variables
(with-params [dict :key] body)       ; Shorthand for (get dict :key)
```

---

## Common Implementation Mistakes

### Choosing Between `def`, `defn`, and `let`

Use `def` for global constants, `defn` for functions, `let` for local bindings.

```sheaf
(def lr 0.001)                ; Global constant
(def colors ["red" "green"])  ; Global list

(defn model [x params] ...)   ; Named function

(let [tmp (* x 2)] ...)       ; Local binding (scoped)
```

### Using `list` Instead of `[]`

Sheaf uses `[]` for vectors/tensors, not `list`.

```sheaf
;; WRONG:
(list 1 2 3)             ; ERROR: no `list` function

;; RIGHT:
[1 2 3]                  ; Vector literal
'[3 4]                   ; Quoted shape (for reshape)
{:a 1 :b 2}              ; Dict literal
```

### Unnecessary `cast` for Typed Vectors

Sheaf vectors support inline dtype annotation. No `cast` needed at declaration.

```sheaf
;; WRONG (PyTorch habit):
(cast [0 1 2] :i32)          ; Unnecessary function call

;; RIGHT:
[0 1 2] :i32                 ; Dtype annotation follows the vector
[1.0 2.0] :bf16              ; BFloat16
[1 2 3]                      ; Default: f32
```

`cast` is only needed to convert an _existing_ tensor to a different dtype:
`(cast existing-tensor :f32)`.

### Missing Parentheses Around `defn`

**Wrong:**

```sheaf
defn forward [x p]
  (with-params [p] (+ (@ x W) b))
```

**Right:**

```sheaf
(defn forward [x p]
  (with-params [p] (+ (@ x W) b)))
```

Sheaf is a Lisp. **Every form must be parenthesized**.

---

### Confusing `as->` and `let`

**`let` creates independent local variables:**

```sheaf
(let [x-input (forward inputs params)
      y-target (get batch :labels)
      lr 0.001]
  ...)
```

**`as->` threads a single value through transformations:**

```sheaf
(as-> (forward inputs params) x
  (relu x)
  (reshape x '[batch -1])
  (softmax x :axis -1)
  (mean x :axis 0))
```

| Task                                 | Use    |
| ------------------------------------ | ------ |
| Multiple independent bindings        | `let`  |
| Single value through transformations | `as->` |
| Single value, first-arg threading    | `->`   |
| Single value, last-arg threading     | `->>`  |

---

### Initializing Adam State Incorrectly

**Wrong:**

```sheaf
(let [m {:w 0.0 :b 0.0}
      v {:w 0.0 :b 0.0}]
  (adam-step params grads m v t lr))
```

**Right:**

```sheaf
(let [m (tree-map-zeros params)
      v (tree-map-zeros params)]
  (adam-step params grads m v t lr))
```

Adam optimizer states must match the nested structure of your parameters.

---

### Using Unquoted Shapes

If the vector contains symbols, it evaluates as a tuple:

```
(reshape x [1 T D])   ; OK — T and D are symbols, result is a shape tuple
(reshape x [3 4])     ; WRONG — all literals → tensor, not shape
(reshape x '[3 4])    ; RIGHT
```

---

### Forgetting Parameter Structure in `with-params`

**Wrong:**

```sheaf
(with-params [params]
  (+ (@ x w) b))      ; If params is {:l1 {:w W :b b}}, w and b aren't in scope
```

**Right:**

```sheaf
(with-params [params :l1]
  (+ (@ x w) b))
```

---

## Debugging & Introspection

### CLI Usage

```bash
sheaf                              # Launch interactive REPL
sheaf file.shf                     # Run a Sheaf file
sheaf -c '(+ 1 2)'                # Evaluate an expression
sheaf file.shf --trace             # With call tracing (--trace fn1,fn2 for specific functions)
sheaf file.shf --blame             # With profiling (wall time per function)
sheaf file.shf --guard no-nan      # CLI guard: halt if any function returns NaN/Inf
sheaf file.shf --device metal      # Target specific device (cpu, metal, cuda, vulkan)
sheaf file.shf -v                  # JIT verbose output
sheaf init-ai                      # Generate sheaf-context.md for AI assistants
```

### JIT Compilation

Pure functions are compiled automatically on first call. No explicit build step
is needed.

Compiled artifacts (VMFB + `manifest.json`) are cached in the `__sheaf__/`
directory beside the source file. The manifest records function signatures and
content hashes; stale entries are recompiled automatically.

The compiler toolchain (`iree-compile`) is downloaded to `~/.sheaf/toolchain/`
on first use.

### Interactive REPL

```bash
sheaf
```

Features:

- Expression evaluation with tensor statistics
- `:env` to see all functions/variables
- Command history saved to `~/.sheaf_history`
- Ctrl-D or `:quit` to exit

### Enable Tracing (Debug Mode)

`--trace` logs every function call with argument shapes and return values. This
is the primary debugging tool when training doesn't converge.

```bash
# Trace all functions
sheaf run.shf --trace

# Trace specific functions only
sheaf run.shf --trace forward,train-step
```

Output shows call tree with shapes, making it easy to spot shape mismatches or
unexpected values without adding `print` statements.

### Use Guards for Runtime Validation

Sheaf has two guard mechanisms. **Inline guards** are written in source code and
are always active; **CLI guards** are specified on the command line and check
function return values without modifying code.

**Inline guards** (permanent, in source):

```sheaf
(guard :no-nan x)                    ; Fail if x contains NaN/Inf
(guard :range [-1.0 1.0] x)         ; Fail if x outside range
(guard :shape [32 128] x)           ; Fail if x doesn't match shape
```

**CLI guards** (temporary, no code changes):

```bash
sheaf train.shf --guard no-nan              # Check all function returns for NaN/Inf
sheaf train.shf --guard forward:no-nan       # Check only `forward` return value
sheaf train.shf --guard loss:range:0:20     # Check `loss` return stays in [0, 20]
```

**Practical debugging example:**

```sheaf
(let [x input
      _ (guard :shape [32 784] x)

      h (with-params [params :l1]
          (relu (+ (@ x W) b)))
      _ (guard :range [-10.0 10.0] h)

      output (with-params [params :l2]
               (sigmoid (+ (@ h W) b)))
      _ (guard :no-nan output)]

  output)
```

### Inspect PyTree Structure

```sheaf
;; Get shapes and dtypes of all parameters
(tree-map (fn [x] (shape x)) params)
; => {:l1 {:w [128 64], :b [64]}, :l2 {:w [10 128], :b [10]}}

;; Get min/max values
(tree-map (fn [x] {:min (min x) :max (max x) :mean (mean x)}) params)
```

### Finding Function Documentation

All function signatures are built into the binary. Use the REPL to look them up:

```bash
echo ':h repeat' | sheaf 2>/dev/null     # Signature + examples
echo ':list' | sheaf 2>/dev/null         # All functions by category
```

---

## Advanced Topics: Shapes, State Threading & Limitations

### Quoted Forms & Static Shapes

Sheaf uses quoted forms for shapes because they must be compile-time constants:

```sheaf
;; WRONG - Cannot evaluate variables inside quotes
(let [hidden_size 8]
  (zeros '[hidden_size]))  ; Error: 'hidden_size' is not a static value

;; CORRECT - Use static shape values
(zeros '[8])

;; CORRECT - Extract shape from existing tensors
(let [h0 (zeros (shape (get params :W_hh)))]
  ...)
```

**Key insight:** Variables inside quotes are never evaluated. If you need
dynamic shapes, extract them from existing tensors using `(shape tensor)`.

### Understanding `scan`: State Threading

`scan` threads state through a sequence. The callback returns
`[new_state, output]`:

```sheaf
(scan (fn [state x] [(+ state x) (+ state x)])
      0.0 [1.0 2.0 3.0])
; => [6.0, [1.0 3.0 6.0]]
```

First return element becomes next iteration's state, second is accumulated. See
the Reference section below for detailed examples including RNN patterns and
multi-component state.

### with-params: Accessing Nested Parameters

`with-params` only works one level deep:

```sheaf
;; Access layer1 parameters
(with-params [params :layer1]
  (+ W b))  ; W and b from :layer1 are bound

;; Two-step access for multiple layers
(with-params [params :layer1]
  (let [hidden (+ (@ x W) b)]
    (with-params [params :layer2]
      (+ (@ hidden W) b))))
```

### Validation Patterns

**Validate state shape at each step:**

```sheaf
(fn [h_prev x_t]
  (let [_ (guard :shape [8] h_prev)
        _ (guard :shape [1] x_t)
        h_next (tanh (+ (@ W_hh h_prev) (@ W_xh x_t) b_h))
        _ (guard :shape [8] h_next)]
    [h_next h_next]))
```

**Verify gradients flow backward through time:**

```sheaf
(let [loss1 (loss-fn params)
      params-eps (tree-map (fn [x] (+ x 1e-4)) params)
      loss2 (loss-fn params-eps)
      diff (abs (- loss2 loss1))]
  (guard :range [1e-8 1.0] diff))
```

---

## Best Practices

### 1. Use `sigmoid` for independent probabilities

```sheaf
(sigmoid attention-scores)  ; Each object can be selected independently
```

### 2. Use `softmax` for exclusive selection

```sheaf
(softmax logits :axis -1)  ; Only one class selected
```

### 3. Use `...` (ellipsis) for rank-agnostic code

```sheaf
(einsum "... i j, ... j k -> ... i k" A B)  ; Works for any batch dims
```

### 4. Use `if` for control-flow, `where` for data-flow

```sheaf
; Control-flow:
(if (= epoch 0) init-lr (* lr 0.1))

; Data-flow (differentiable, tensor-level):
(where (> x 0) x (* x 0.01))  ; Leaky ReLU
```

### 5. Leverage `with-params` for cleaner code

```sheaf
; Instead of:
(let [W (get params :W) b (get params :b)]
  (+ (@ x W) b))

; Write:
(with-params [params]
  (+ (@ x W) b))

; Or with key shorthand:
(with-params [params :l1]
  (+ (@ x W) b))
```

---

**End of AI Context Reference** • For function signatures, use the REPL (see
below).

---

## Function Reference

The complete function reference is built into the Sheaf binary and accessible
via the REPL.

**List all functions by category:**

```bash
echo ':list' | sheaf 2>/dev/null
```

**Show signature, description, and examples for a function:**

```bash
echo ':h <name>' | sheaf 2>/dev/null
```

**Tip for AI agents:** Run `:list` once to get the full index. Then use `:h` for
any function whose signature you need. Redirect stderr to get clean output:

```bash
echo ':list' | sheaf 2>/dev/null > /tmp/sheaf-functions.txt
echo ':h adam-step' | sheaf 2>/dev/null >> /tmp/sheaf-functions.txt
```
